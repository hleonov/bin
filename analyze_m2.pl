#!/usr/bin/perl -w
# This script should eventually run all the analysis scripts possible on a simulation
# in my case it's M2, hence the specific name. .
...
sub P_prot {
	my $com = "printf \"a P\nq\n\" | make_ndx -f $param{'mdtpr'} -n $param{'index'} -o ../system3.ndx";
#	my $com2 = "printf \"P\nTM_region\n\" | g_dist -f $param{'trj'} -n ../system3.ndx -s $param{'mdtpr'} -o P_protCOM_dist.xvg ";
	my $com2 = "printf \"P\n1\n\" | g_dist -f $param{'trj'} -n ../system3.ndx -s $param{'mdtpr'} -o P_protCOM_dist.xvg ";

	system($com) == 0 || die "P-protein dist proc: make-index Error\n";
	system($com2) == 0 || die "P-protein dist proc: g_dist Error\n";
}
...
sub aa_dist {
	my $com1 = "$vmdt -e ~/vmd_scripts/z_aa.tcl -args $param{'pr'} $param{'trjtype'} $param{'conskip'} $param{'trj'} $param{'ch_len'} $param{'aa_group'}";

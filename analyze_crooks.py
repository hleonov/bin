...
def dG_from_overlap(data1, data2, fname, dpi, T = 298):
    maxi = max( data1+data2 )
    mini = min( data1+data2 )    
    nf, bins1, patches1 = hist(data1, range = (mini,maxi),bins=100, facecolor='blue', alpha=0.75, normed=True, label='0->1')
    nb, bins2, patches2 = hist(data2, range = (mini,maxi),bins=100, facecolor='red', alpha=0.75, normed=True, label='1->0')
    ...
    for i in range(len(nf)):
	   ...
	   x.append( bins1[i] )
       	   #print nf[i], nb[i]
    print "  Number of overlapping points: ", len(x)
    if (len(x) > 0):
       fit = polyfit(x,y,1)  #fit = array([a,b]) (ax+b)
       ...
       return  -fit[1]/fit[0] , fit[0]	# y=ax+b, dG is intersection with x (so y=0, x= -b/a )
    else:
...
def main(argv):
    ...
    overlap_dG_result, oBeta = dG_from_overlap(res_ab, res_ba, "overlap_reg.png", cmdl['-dpi'], T)
    tee(out, '  RESULT: dG ( overlap )  = %8.4f kJ/mol' % overlap_dG_result)
    kb=0.00831447215
    
    tee(out, '  RESULT: Beta ( overlap ) vs. beta  = %3.3f %3.3f' % (oBeta, 1./(kb*T)) )
    ##
    if intersection:

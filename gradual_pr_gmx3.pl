...
while ($f>=$min_f) {
	...
	if ($f == $max_f && !$recover) {
		system("grompp -f $pr_file -c $em_file -n system.ndx -p system.top -o system_for_PR.tpr -np $np") == 0
			|| die "Cannot execute grompp\n";
		
	} else {
		$i=$f+$df;
		system("grompp -f $pr_file -c system_from_PR$i.gro -n system.ndx -p system.top -o system_for_PR.tpr -np $np") == 0
			|| die "Cannot execute grompp\n";
	}

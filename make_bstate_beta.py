...
from pymacs_beta.forcefield2 import Topology
from pymacs_beta.mutdb import read_mtp_entry
            ...
            for dx in topol.dihedrals:
                ...
                if (dx[0].id == al[0].id and \
                   ...
                   (dx[0].id == al[3].id and \
                    ...
                    dx[3].id == al[0].id):
                    A,B =  check_case(al[:4])
                    if d[4].startswith('default'):
                        if A=='AAAA':
                            astate = topol.BondedParams.get_dihedral_param(al[0].type,al[1].type,al[2].type,al[3].type, func)
                            if astate is None:
                        ...
                        astate = [3,0,0,0,0,0,0]
                    else:
                        astate = d[4]
                    if d[5].startswith('default'):
                        if B=='AAAA':
#                            print al[0].typeB,al[1].typeB,al[2].typeB,al[3].typeB
                            bstate = topol.BondedParams.get_dihedral_param(al[0].typeB,al[1].typeB,al[2].typeB,al[3].typeB, func)
                            if bstate is None:
...
def get_hybrid_residues( m, mtp_file ):
    rdic = {}
    rlist = []
    for res in m.residues:
        if is_hybrid_residue( res.resname ):
            rlist.append( res )
            mtp = get_hybrid_residue( res.resname, mtp_file )

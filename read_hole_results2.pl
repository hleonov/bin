...
$file = shift || die "Usage: perl $0 <hole log> <out> <dz>\n";
$out = shift  || die "Usage: perl $0 <hole log> <out> <dz>\n";
$dz = shift   || die "Usage: perl $0 <hole log> <out> <dz>\n";
undef(@rad);
$SM_CUTOFF = 2;
$endhist_cutoff = 6;
$z_min = 10000;
...
sub fix_matrix {	
	...
	for ($frame=0; $frame<=$#hole; $frame++) {
		...
		for ($z=$z_min; $z<=$z_max; $z++) {
			if (defined $hole[$frame][$z]) {	
				if ($num[$frame][$z] >= $endhist_cutoff) {
				    $rad[$frame][$z] = $hole[$frame][$z] / $num[$frame][$z];
					#print "frame: $frame	z: $z    num: $num[$frame][$z]    rad: $rad[$frame][$z]\n";
					$sum[$z] += $rad[$frame][$z];
					$num_avg[$z]++;
				}
			}
	...
	for ($z=$z_min+1; $z<$z_max-1; $z++) {
		print "z: $z    sum: $sum[$z]    num_avg: $num_avg[$z]\n";
	}	
	for ($frame=0; $frame<=$#hole; $frame++) {
		for ($z=$z_min+1; $z<$z_max-1; $z++) {
			if (!defined $rad[$frame][$z]) {
#				print "Z: $z\t num: $num_avg[$z]\t sum $sum[$z]\n";
				 $rad[$frame][$z] = $sum[$z]/$num_avg[$z];
			}
		}
	}
}

...
$inFile = shift || die "Usage: perl $0 <infile> <minwin> <maxwin> <intervals>\n";
$MIN_WIN = shift || die "Usage: perl $0 <infile> <minwin> <maxwin> <intervals>\n";
$MAX_WIN = shift || die "Usage: perl $0 <infile> <minwin> <maxwin> <intervals>\n";
$DW = shift || die "Usage: perl $0 <infile> <minwin> <maxwin> <intervals>\n";

open(OUT,">$inFile.$MIN_WIN.$MAX_WIN.out");
open(IN,"$inFile");
while(<IN>){
  chop;
  if(/^>/){   # it is a new one
...
sub energy {
	...
	my $pr_seq = substr($_[0], $maxPos, $maxWin);
	print OUT "Sequence: $pr_seq\n";
 	print OUT "---------------------------------------------------------\n";
}
